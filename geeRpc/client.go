...
type Call struct {
	...
	Done          chan *Call  //Strobes when call is complete. 呼叫完成时选通。
}
...
// 一个客户端可能有多个未完成的呼叫，并且一个客户端可以同时被多个goroutine使用
...
type Client struct {
	...
	opt      *Option          //是一个互斥锁，和服务端类似，为了保证请求的有序发送，即防止出现多个请求报文混淆。
	...
	closing  bool             //user had called close
...
// terminateCalls 终止通话 服务端或客户端发生错误时调用，将 shutdown 设置为 true，且将错误信息通知所有 pending 状态的 call。
...
// Go invokes the function asynchronously. Go异步调用函数
//
//	It returns the Call structure representing the invocation. 它返回表示调用的Call结构。
